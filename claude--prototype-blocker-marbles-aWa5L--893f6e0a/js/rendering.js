...
function drawStock() {
  for (var i = 0; i < stock.length; i++) {
    ...
    if (b.revealT > 0) {
      ...
      if (b.hasBlockers && phase > 0.3) {
        drawBlockerBoxBadge(-L.bw / 2, -L.bh / 2, L.bw, L.bh, Math.min(1, (phase - 0.3) / 0.5));
      }
    } else if (!b.revealed) {
      ...
      if (b.hasBlockers) {
        drawBlockerBoxBadge(-L.bw / 2, -L.bh / 2, L.bw, L.bh, 0.7);
      }
    } else {
      var c = COLORS[b.ci];
      ...
      if (b.remaining > 0) {
        if (b.hasBlockers) {
          drawBoxMarblesWithBlockers(b.ci, b.remaining, BLOCKER_PER_BOX);
        } else {
      ...
      if (b.hasBlockers) {
        drawBlockerBoxBadge(-L.bw / 2, -L.bh / 2, L.bw, L.bh, 1);
      }
    }
...
function drawBelt() {
  ...
  for (var i = 0; i < BELT_SLOTS; i++) {
    ...
    if (slot.marble >= 0) {
      ...
      if (slot.marble === BLOCKER_CI && blocker.collecting && blocker.collectT > 0.65) {
        var ct = (blocker.collectT - 0.65) / 0.35;
