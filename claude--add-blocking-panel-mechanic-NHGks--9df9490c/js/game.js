...
function initGame() {
  ...
  // ── Panels: init and un-reveal covered boxes ──
  initPanels(lvl);
  for (var pi = 0; pi < panels.length; pi++) {
    var panelCells = panels[pi].cells;
    for (var pc = 0; pc < panelCells.length; pc++) {
      var pb = stock[panelCells[pc]];
      if (pb && !pb.empty && !pb.isTunnel && !pb.isWall) {
        pb.revealed = false;
      }
    }
  }
...
function revealAroundEmptyCell(idx) {
  ...
  for (var ni = 0; ni < neighbors.length; ni++) {
    ...
    if (isCellPaneled(nIdx)) continue;  // panels prevent reveal
    nb.revealed = true;
    nb.revealT = 1.0;
...
function isBoxTappable(idx) {
  ...
  if (isCellPaneled(idx)) return false;  // panels block tapping
  return b.revealed;
}
