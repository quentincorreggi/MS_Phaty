...
function initGame() {
  ...
  // ── Build boxSlots, tunnelSlots, wallSlots, doorSlots from grid ──
  var boxSlots = {};
  var tunnelSlots = {};
  var wallSlots = {};
  var doorSlots = {};        // idx → { doorId, start }
  var doorGroups = {};       // doorId → { start, cellIdxs: [] }
  if (lvl.grid) {
    for (var i = 0; i < Math.min(lvl.grid.length, totalSlots); i++) {
      ...
      if (cell.door) {
        var did = cell.doorId || ('d_' + i);
        doorSlots[i] = { doorId: did };
        if (!doorGroups[did]) doorGroups[did] = { start: cell.start || 'open', cellIdxs: [] };
        doorGroups[did].cellIdxs.push(i);
        // If start state differs across cells of the same group, last writer wins.
        if (cell.start) doorGroups[did].start = cell.start;
        continue;
      }
      if (cell.tunnel) {
  ...
  // Sort each door's cellIdxs left-to-right and assign sequential numeric ids
  var doorList = [];
  var doorIdMap = {};
  var nextDid = 0;
  for (var gKey in doorGroups) {
    var g = doorGroups[gKey];
    g.cellIdxs.sort(function (a, b) { return a - b; });
    doorIdMap[gKey] = nextDid;
    doorList.push({ cellIdxs: g.cellIdxs.slice(), start: g.start });
    nextDid++;
  }
  initDoors(doorList);
  ...
  for (var r = 0; r < L.rows; r++) for (var c = 0; c < L.cols; c++) {
    ...
    if (dSlot) {
      // Door cell — structural, never tappable
      stock.push({
        isDoor: true, isTunnel: false, isWall: false,
        doorId: doorIdMap[dSlot.doorId],
...
function updateBoxReveals(animate) {
  ...
  for (var i = 0; i < total; i++) {
    ...
    if (s.isDoor) {
      var dr = (typeof doors !== 'undefined') ? doors[s.doorId] : null;
      passable[i] = !!(dr && dr.state === 'open');
      continue;
    }
  ...
  for (var k = 0; k < total; k++) {
    var b = stock[k];
    if (!b) continue;
    if (b.isWall || b.isTunnel || b.isDoor || b.empty || b.used) continue;
...
function damageAdjacentIce(idx) {
  ...
  for (var ni = 0; ni < neighbors.length; ni++) {
    ...
    if (nb.isTunnel || nb.isWall || nb.isDoor) continue;  // tunnels, walls, doors don't have ice
...
function isBoxTappable(idx) {
  ...
  if (b.isDoor) return false;      // doors are not tappable
...
function handleTap(px, py) {
  ...
  for (var i = 0; i < stock.length; i++) {
    var b = stock[i];
    if (b.isTunnel || b.isWall || b.isDoor) continue;  // skip structural cells in tap handler
...
canvas.addEventListener('mousemove', function (e) {
  ...
  for (var i = 0; i < stock.length; i++) {
    var b = stock[i];
    if (b.isTunnel || b.isWall || b.isDoor) continue;
    if (b.empty || b.used || b.spawning || b.revealT > 0) continue;
...
function update() {
  ...
  for (var i = 0; i < stock.length; i++) {
    var b = stock[i];
    if (b.isTunnel || b.isWall || b.isDoor) continue;  // structural cells don't need stock animations
    if (b.empty) continue;
  ...
  tickParticles();
  updateRollingSound();
  if (typeof updateDoorAnims === 'function') updateDoorAnims();
}
