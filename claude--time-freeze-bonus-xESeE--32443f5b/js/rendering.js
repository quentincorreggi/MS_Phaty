...
function drawPhysMarbles() {
  for (var i = 0; i < physMarbles.length; i++) {
    ...
    if (m.frozen) ctx.save();
    if (m.frozen) ctx.globalAlpha = 0.4;
    drawMarble(m.x, m.y, m.r, m.ci, bounce);
    if (m.frozen) {
      // Ice glow overlay
      var pulseAlpha = 0.2 + Math.sin(tick * 0.08 + i * 0.5) * 0.1;
      ctx.globalAlpha = pulseAlpha;
      var fGrad = ctx.createRadialGradient(m.x, m.y, 0, m.x, m.y, m.r * 1.3);
      fGrad.addColorStop(0, 'rgba(200,235,255,0.5)');
      fGrad.addColorStop(0.7, 'rgba(126,200,227,0.3)');
