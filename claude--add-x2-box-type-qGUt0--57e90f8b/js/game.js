...
function initGame() {
  ...
  for (var k in boxSlots) {
    ...
    var boxTotal = isX2Box ? MRB_PER_BOX * 2 : MRB_PER_BOX;
  ...
  for (var r = 0; r < L.rows; r++) for (var c = 0; c < L.cols; c++) {
    ...
    if (tSlot) {
      ...
      var boxRemaining = isX2 ? MRB_PER_BOX * 2 : MRB_PER_BOX;
...
function handleTap(px, py) {
  ...
  for (var i = 0; i < stock.length; i++) {
    ...
    if (px >= b.x && px <= b.x + L.bw && py >= b.y && py <= b.y + L.bh) {
      ...
      b.popT = 1;
      sfx.pop();
      var burstN = b.boxType === 'x2' ? 30 : 18;
      spawnBurst(b.x + L.bw / 2, b.y + L.bh / 2, COLORS[b.ci].fill, burstN);
      spawnPhysMarbles(b);
      damageAdjacentIce(i);
      return;
    }
