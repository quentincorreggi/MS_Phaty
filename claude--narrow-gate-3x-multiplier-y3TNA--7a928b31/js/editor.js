...
function editorRenderGrid() {
  ...
  for (var i = 0; i < 49; i++) {
    ...
    if (v && v.wall) {
      ...
      // Gate cell
      cell.style.background = 'linear-gradient(135deg,#7BE89E,#2EA858)';
      cell.style.borderColor = '#1F8A48';
      cell.innerHTML = '<span class="ed-cell-dot" style="color:#fff;font-size:11px;font-weight:700">&#215;3</span>';
    } else if (v && v.tunnel) {
      // Tunnel cell
...
function editorRenderToolbar() {
  ...
  gateBtn.textContent = '\u00D73 Gate';
  gateBtn.style.borderColor = editor.gateMode ? 'rgba(46,168,88,0.7)' : '';
  gateBtn.style.color = editor.gateMode ? '#2EA858' : '';
  ...
  if (editor.tunnelMode) {
    ...
    gateInfo.innerHTML = '<span style="font-size:11px;color:#9C8A70">Click cells to place/remove ×3 gates &middot; marbles passing through are tripled</span>';
...
function editorUpdateStats() {
  ...
  if (gateCount > 0) {
    html += '<span class="ed-stat-chip" style="background:#2EA858">' + gateCount + ' &times;3 gate' + (gateCount > 1 ? 's' : '') + '</span>';
  }
  if (tunnelCount > 0) {
