// ============================================================
// drill.js — Drill & Rail mechanic (v2)
// A drill sits on a rail (connected chain of boxes) and can
// destroy one wall in its pointed direction.
// V2: Tapping ANY rail box activates the drill at its current
// position. Non-rail box taps still move the drill one step
// along the rail (ping-pong).
// ============================================================
...
function drillActivate() {
  ...
  if (targetRow >= 0 && targetRow < L.rows && targetCol >= 0 && targetCol < L.cols) {
    ...
    if (stock[targetIdx] && stock[targetIdx].isWall) {
      ...
      // Reveal boxes around the destroyed wall
      _revealVisited = {};
