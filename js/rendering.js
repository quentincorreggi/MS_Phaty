...
function drawStock() {
  for (var i = 0; i < stock.length; i++) {
    ...
    var pdx = b.drawDx || 0, pdy = b.drawDy || 0; // spinning-plate slide offset
    var breathe = 0;
    ...
    // Empty slot
    if (b.empty) { drawEmptySlot(b.x + pdx, b.y + pdy, L.bw, L.bh); continue; }

    // Used box fading out
    if (b.used && b.emptyT > 0) {
      ts *= 0.7 + 0.3 * (1 - b.emptyT);
      ctx.save(); ctx.globalAlpha = 1 - b.emptyT * 0.3;
      ctx.translate(b.x + L.bw / 2 + ox + pdx, b.y + L.bh / 2 + pdy); ctx.scale(ts, ts);
      drawEmptySlot(-L.bw / 2, -L.bh / 2, L.bw, L.bh);
      ctx.restore(); continue;
    }

    // Used box (fully empty)
    if (b.used) { drawEmptySlot(b.x + pdx, b.y + pdy, L.bw, L.bh); continue; }
