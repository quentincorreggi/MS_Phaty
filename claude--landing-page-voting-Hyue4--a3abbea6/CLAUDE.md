...
## Notion Voting Integration

The landing page and deploy workflow support automatic syncing of prototypes
to a Notion database, enabling the team to vote and comment on prototypes.

### How It Works

- Every deploy creates or updates a page in a shared Notion database
- Each page has: Name, Description, Author, Branch, Playable URL, SHA, Status
- When a branch is deleted, the Notion page is marked as "Archived"
- The landing page shows a "Vote on Notion" button linking to the database

### Setup (one-time)

1. **Create a Notion integration:**
   - Go to https://www.notion.so/my-integrations
   - Click "New integration", name it "Marble Sorter CI"
   - Copy the "Internal Integration Secret" (starts with `ntn_`)

2. **Create the Notion database:**
   - Create a new full-page database in your team workspace
   - Add these properties (exact names matter):
     ...
     - `Playable URL` (URL)
     - `SHA` (Text)
     - `Status` (Select) — add options: "Active", "Archived"
   - Click the "..." menu → "Connections" → add the "Marble Sorter CI" integration

3. **Get the database ID:**
   - Open the database page in Notion
   - The URL looks like: `https://www.notion.so/<workspace>/<database-id>?v=...`
...
4. **Add GitHub repo secrets** (Settings → Secrets → Actions):
   ...
   - `NOTION_PAGE_URL` — the public URL to your Notion database page
     (e.g., `https://www.notion.so/yourteam/Prototype-Voting-abc123`)

Once configured, every future deploy will auto-sync to Notion. Existing
prototypes will sync on their next push.
