...
All prototypes using the `prototype/` branch prefix are automatically listed
on the landing page at:
...
**IMPORTANT:** Prototype branches MUST use the `prototype/<slug>` prefix to
appear on the landing page. Branches with other prefixes (e.g., `claude/`)
will not be listed.
