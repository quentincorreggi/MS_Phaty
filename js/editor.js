...
// A tall box occupies one cell like any other box — it just holds
// TALL_CELLS times the marbles, which the stats panel accounts for.
    ...
    if (v.tunnel) {
      ...
      if (v.contents) {
        ...
        for (var tc = 0; tc < v.contents.length; tc++) {
          ...
          } else if (editorIsTallType(tItem.type)) {
            regularMrb[tItem.ci] += editor.mrbPerBox * TALL_CELLS;
          } else {
