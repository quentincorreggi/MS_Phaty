...
function drawBoxMarblesX2(ci, remaining) {
  if (remaining <= 0) return;
  var mr = Math.min(6 * S, L.bw / 10);
  var mg = Math.min(12 * S, L.bw / 4.8);
  var mgY = mg * MRB_GAP_FACTOR;
  var totalForBox = MRB_PER_BOX * 2;
  var gone = totalForBox - remaining;
  // Draw in two layers: bottom layer (indices 0-8) then top layer (9-17)
  var mrbsToDraw = [];
  for (var si = gone; si < totalForBox; si++) {
    var layer = Math.floor(si / 9);
    var posIdx = si % 9;
    mrbsToDraw.push({ r: SNAKE_ORDER[posIdx].r, c: SNAKE_ORDER[posIdx].c, layer: layer });
  }
  // Sort by row (back to front) then layer
  mrbsToDraw.sort(function (a, b) { return a.r === b.r ? a.layer - b.layer : a.r - b.r; });
  for (var si = 0; si < mrbsToDraw.length; si++) {
    var sp = mrbsToDraw[si];
    var ox = sp.layer * 2 * S;
    var oy = sp.layer * (-3 * S);
    drawMarble((sp.c - 1) * mg + ox, (sp.r - 1) * mgY - 2 * S + oy, mr, ci);
  }
}
    ...
    if (b.revealT > 0) {
      ...
      if (b.remaining > 0) {
        if (b.boxType === 'blocker' && b.blockerCount > 0) {
          ...
          drawBoxMarblesX2(b.ci, b.remaining);
        } else {
