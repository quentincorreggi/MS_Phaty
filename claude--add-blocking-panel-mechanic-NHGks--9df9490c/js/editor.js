...
var editor = {
  ...
  panelMode: false,     // true when placing 2x2 panels
  panels: [],           // array of anchor indices for 2x2 panels
  visible: false
};
...
function editorRenderGrid() {
  ...
  for (var i = 0; i < 49; i++) {
    ...
    // Panel overlay
    if (editorCellHasPanel(i)) {
      var overlay = document.createElement('div');
      overlay.style.cssText = 'position:absolute;inset:0;background:rgba(107,80,32,0.55);border-radius:4px;display:flex;align-items:center;justify-content:center;pointer-events:none';
      overlay.innerHTML = '<span style="color:rgba(255,255,255,0.8);font-size:10px;font-weight:700;text-shadow:0 1px 2px rgba(0,0,0,0.3)">P</span>';
...
// ── Panel helpers ──
function editorCellHasPanel(idx) {
  var row = Math.floor(idx / 7), col = idx % 7;
  for (var i = 0; i < editor.panels.length; i++) {
    var ar = Math.floor(editor.panels[i] / 7);
    var ac = editor.panels[i] % 7;
    if (row >= ar && row <= ar + 1 && col >= ac && col <= ac + 1) return true;
  }
  return false;
}

function editorRemovePanelAt(idx) {
  var row = Math.floor(idx / 7), col = idx % 7;
  for (var i = editor.panels.length - 1; i >= 0; i--) {
    var ar = Math.floor(editor.panels[i] / 7);
    var ac = editor.panels[i] % 7;
    if (row >= ar && row <= ar + 1 && col >= ac && col <= ac + 1) {
      editor.panels.splice(i, 1);
      return true;
    }
  }
  return false;
...
function editorCanPlacePanel(idx) {
  var row = Math.floor(idx / 7), col = idx % 7;
  // Must fit in grid
  if (row + 1 >= 7 || col + 1 >= 7) return false;
  // All 4 cells must have boxes (not empty, not tunnel, not wall)
  var cells = [idx, idx + 1, idx + 7, idx + 8];
  for (var i = 0; i < cells.length; i++) {
    var v = editor.grid[cells[i]];
    if (!v || v.tunnel || v.wall || v.ci === undefined) return false;
  }
  // Must not overlap existing panels
  for (var i = 0; i < cells.length; i++) {
    if (editorCellHasPanel(cells[i])) return false;
  }
  return true;
}
...
function editorCellClick(e) {
  ...
  if (editor.panelMode) {
    ...
    if (editorCellHasPanel(idx)) {
      // Remove panel covering this cell
      editorRemovePanelAt(idx);
    } else if (editorCanPlacePanel(idx)) {
      editor.panels.push(idx);
    } else {
      editorShowToast('Panel needs a 2x2 area of boxes');
    }
    editorRenderGrid();
    editorUpdateStats();
    return;
...
function editorRenderToolbar() {
  ...
  panelBtn.style.borderColor = editor.panelMode ? 'rgba(139,105,20,0.6)' : '';
  panelBtn.style.color = editor.panelMode ? '#8B6914' : '';
  ...
  } else if (editor.panelMode) {
    // Panel mode: info hint
    var panelInfo = document.createElement('div');
    panelInfo.className = 'ed-color-row';
    panelInfo.innerHTML = '<span style="font-size:11px;color:#9C8A70">Click a cell to place a 2\u00d72 panel (top-left corner). Click a paneled cell to remove.</span>';
...
function editorUpdateStats() {
  ...
  if (editor.panels.length > 0) {
    html += '<span class="ed-stat-chip" style="background:#8B6914">' + editor.panels.length + ' panel' + (editor.panels.length > 1 ? 's' : '') + '</span>';
  }
  if (tunnelCount > 0) {
...
function editorBuildLevel() {
  ...
  if (editor.panels.length > 0) {
    lvl.panels = editor.panels.map(function (a) { return { anchor: a }; });
  }
  return lvl;
}
...
function editorImportJSON() {
  ...
  if (ta.style.display === 'block' && ta.value.trim()) {
    try {
      ...
      if (lvl.panels) {
        editor.panels = lvl.panels.map(function (p) { return p.anchor; });
      } else {
        editor.panels = [];
      }
