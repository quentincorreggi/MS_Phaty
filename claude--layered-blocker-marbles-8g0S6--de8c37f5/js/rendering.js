...
function drawStock() {
  for (var i = 0; i < stock.length; i++) {
    ...
    if (b.revealT > 0) {
      ...
      if ((b.boxType === 'blocker' || b.boxType === 'layered_blocker') && b.blockerCount > 0) {
        ctx.save();
...
function drawBelt() {
  ...
  for (var i = 0; i < BELT_SLOTS; i++) {
    ...
    if (slot.marble >= 0) {
      ...
      var activeCollectT = 0;
      if (blockerCollecting && blockerCollectT > 0.5) activeCollectT = blockerCollectT;
      if (layeredBlockerCollecting && layeredBlockerCollectT > 0.5) activeCollectT = layeredBlockerCollectT;
      if (slot.marble === BLOCKER_CI && activeCollectT > 0) {
        var ct = (activeCollectT - 0.5) * 2;
...
function drawLayeredBlockerProgress() {
  if (!isLayeredBlockerLevel) return;
  var filled = 0;
  for (var i = 0; i < BELT_SLOTS; i++) {
    if (beltSlots[i].marble === BLOCKER_CI) filled++;
  }
  filled = Math.min(filled, LAYERED_BLOCKER_THRESHOLD);
  var cx = L.beltCx;
  var cy = (L.beltBotY + L.sTop) / 2;
  var total = LAYERED_BLOCKER_THRESHOLD;
  var dotR = 3.5 * S;
  var gap = dotR * 3;
  var startX = cx - (total - 1) * gap / 2;
  var pillW = Math.max((total - 1) * gap + dotR * 5, dotR * 6);
  ...
  for (var i = 0; i < total; i++) {
    ...
    if (i < filled) {
      ...
      ctx.fillStyle = 'rgba(255,255,255,0.25)';
      ctx.beginPath(); ctx.arc(dx - dotR * 0.2, cy - dotR * 0.2, dotR * 0.35, 0, Math.PI * 2); ctx.fill();
      if (layeredBlockerCollecting && layeredBlockerCollectT > 0.5) {
        ctx.globalAlpha = 0.4 + Math.sin(tick * 0.2 + i * 0.5) * 0.3;
        ctx.fillStyle = bc.glow;
        ctx.beginPath(); ctx.arc(dx, cy, dotR * 2, 0, Math.PI * 2); ctx.fill();
        ctx.globalAlpha = 1;
  ...
  if (layeredBlockerCollecting && layeredBlockerCollectT <= 0.5 && layeredBlockerCollectT > 0) {
    var flashAlpha = layeredBlockerCollectT * 2;
    ctx.globalAlpha = flashAlpha * 0.6;
    ctx.fillStyle = 'rgba(255,255,255,0.8)';
    rRect(cx - pillW / 2, cy - pillH / 2, pillW, pillH, pillH / 2); ctx.fill();
    ctx.globalAlpha = 1;
