...
3. Create or verify branch:
   - If already on a platform-assigned branch (e.g., `claude/...`), stay on it.
     Verify the branch name contains "prototype" — if not, warn the user that
     the prototype won't appear on the landing page (see Prototype Library
     Landing Page section below).
   - Otherwise: `git checkout main && git checkout -b prototype/<slug>`
...
**IMPORTANT — Branch naming requirement:** When running on platforms that
control branch naming (e.g., Claude Code web assigns `claude/<description>-<id>`),
the word "prototype" must appear somewhere in the branch name. If it doesn't,
the deploy will succeed but the prototype **will not** be listed on the landing
page. Before coding, always verify the current branch name contains "prototype".
If it doesn't, warn the user early.
